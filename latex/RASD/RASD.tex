...
\begin{description}
  ...
  \item[System:] the automatic part that manages the service.  % (?) <- add doubts/TODOs as comments pls----> is that the right definition?
...
\section{Assumption}
\begin{itemize}
\item If a request comes from a zone, which queue is empty, the system forwards the call to the first taxi in the queue corresponding to an adiacent taxi zone, strartin from the northest.
\item A passenger doesn't need of an account to reserve a taxi, unless he wants to add a reservation.%team discussion--->confirm/change this assumption
\item Taxi drivers can create only one account per vichle ID %<--Targa
...
\item Passengers go in the same direction if at least one of the following constraints is true: 
\begin{itemize}
\item one destination is included in in the path generated to reach the others
\item 
\end{itemize}
\end{itemize}
