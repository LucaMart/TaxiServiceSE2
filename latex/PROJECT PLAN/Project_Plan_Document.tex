\documentclass[a4paper,11pt,fleqn]{report}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{listings}
\usepackage{rotating}
\usepackage[hidelinks]{hyperref}
\usepackage[space]{grffile}
\usepackage{amsmath}
\usepackage{array}
\usepackage{longtable}
% Title Page
\title{\textbf{Test Plan Document}}
\author{
  Michele Madaschi
  \and
  Lidia Moioli
  \and
  Luca Martinazzi  % last but not least
}

\begin{document}
\maketitle
\tableofcontents

\chapter*{Introduction}
\addcontentsline{toc}{chapter}{Introduction}
\setcounter{section}{0}
\addtocounter{chapter}{1}
%\input{chapters/introduction.tex}
 
%Internal Logic File: users(x3), ride, sharedride, taxiqueue 
%External Interface File: gps coordinates,
%External Imput: login,logout, request, reserve,delete, reserve shared, accept call, refuse call
%External Output:message (eta, no taxi message)
%External Inquiry: see profile, see active ride list
\chapter*{Function Points} 
\addcontentsline{toc}{chapter}{Function Points}
\setcounter{section}{0}
\addtocounter{chapter}{1}
%\input{chapters/integration_strategy.tex}

%effort=2.94*EAF*(KLOC)^E
%duration=3.67*(effort)^Se
%Npeople=effort/duration
\chapter*{COCOMO II}
\addcontentsline{toc}{chapter}{COCOMO II}
\setcounter{section}{0}
\addtocounter{chapter}{1}
%\input{chapters/individual_steps_and_test_description.tex}

 \chapter*{Schedule and resources allocation}
 \addcontentsline{toc}{chapter}{Schedule and resources allocation}
 \setcounter{section}{0}
 \addtocounter{chapter}{1}
 %\input{chapters/tools_and_testing_equipment_required.tex}

 \chapter*{Risk evaluation}
 \addcontentsline{toc}{chapter}{Risk evaluation}
 \setcounter{section}{0}
 \addtocounter{chapter}{1}
 %\input{chapters/tools_and_testing_equipment_required.tex}

\end{document}    